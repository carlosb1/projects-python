...
class VGG16ImageSearchEngine(object):
    ...
    def __init__(self, folder_from_load='uploads'):
        self.fe = VGG16FeatureExtractor()
        self.img_features = []
        if not os.path.exists(folder_from_load):
            os.makedirs(folder_from_load)
...
def main():
    ...
    # search_engine.index_images([os.path.join(DATA_DIR_PATH, 'Pokemon1.jpg'), os.path.join(DATA_DIR_PATH, 'Pokemon2.jpg')])
    # print(search_engine.rank_top_k(os.path.join(DATA_DIR_PATH, 'Pokemon1.jpg'), k=30))
    pass
