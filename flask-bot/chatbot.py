...
# wrapper to call function as an object
class WrapperChatbot:
    ...
    def __init__(self):
        self.model_path, self.config_path, self.vocab_path = get_paths(self.save_dir)
        ...
        self.saved_args.batch_size = self.beam_width
        self.net = Model(self.saved_args, True)
        if self.not_gpu:
    ...
    def run(self, input_text):
        return(internal_chatbot(self.net, self.sess, self.chars, self.vocab, self.n, self.beam_width, self.relevance, self.temperature, self.topn, input_text))
...
def run_wrap(input_text=''):
    ...
    with tf.Session(config=config) as sess:
        ...
        return(internal_chatbot(net, sess, chars, vocab, n, beam_width,
                relevance, temperature, topn, input_text))
...
def internal_chatbot(net, sess, chars, vocab, max_length, beam_width, relevance, temperature, topn, user_input):
